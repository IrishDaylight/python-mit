...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(26):
            ...
            newStr = self.apply_shift(26 - s)

            for word in newStr.split(" "):
                if is_word(self.valid_words, word):
                    goodWords += 1

            goodWordsList[s] = goodWords

        bestShift = 26 - max(goodWordsList.keys(), key=(lambda k: goodWordsList[k]))
        if bestShift == 26:
            bestShift = 0

        return tuple([bestShift, self.apply_shift(bestShift)])

def decrypt_story():
    """
    returns: appropriate shift value and decrypted story
    """
    story = get_story_string()
    dStory = CiphertextMessage(story)
    print dStory.decrypt_message()
